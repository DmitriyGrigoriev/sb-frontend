  ...
  <q-item flat bordered>
    <q-item-section avatar>
      <q-avatar class="text-white text-uppercase">
        <q-img
          :src="
            user.image_url ? user.image_url : 'avatar-placeholder.png'
          "
        />
      </q-avatar>
    </q-item-section>
    ...
    <q-item-section>
      <div v-for="role in user.role" :key="role.id" class="row q-mt-md justify-center">
        <q-badge>{{ role.name }}</q-badge>
      </div>
...
export default {
  ...
  computed: {
    ...
    roleIds () {
      const Ids = []
      const forFn = function (userRoles) {
        for (const item of userRoles) {
          Ids.push(item.id)
        }
      }
      forFn(this.user.role)
      return Ids
    }
  },
  data () {
